      ...
      <div className="max-w-xl w-full space-y-8 animate-in fade-in zoom-in-95">
        ...
        <div className="bg-[#252630] p-6 rounded-3xl border border-slate-700 shadow-2xl space-y-6">
          {onboardingStep === 1 ? (
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4">
              <div className="flex justify-between items-center bg-[#1a1b23] p-3 rounded-2xl border border-slate-800"><h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2"><Calculator className="w-4 h-4 text-amber-400" /> Define Strings</h3><button onClick={addString} className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-sm transition-all active:scale-95"><Plus className="w-3.5 h-3.5" /> Add String</button></div>
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6">
        ...
        {showConfig && (
          <div className="bg-[#252630] p-5 rounded-xl border border-slate-700 shadow-lg animate-in fade-in slide-in-from-top-4 space-y-6">
            <div className="flex bg-[#1a1b23] p-1 rounded-xl border border-slate-800 mb-4">
        ...
        {activeTab === 'today' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className={`bg-gradient-to-br from-[#1e293b] to-[#0f172a] p-5 rounded-xl border shadow-sm flex flex-col justify-between transition-all duration-500 ${needsEntry && !isDemo ? 'border-amber-500/50 ring-2 ring-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-indigo-500/30'} ${isDemo ? 'opacity-50 cursor-not-allowed' : ''}`}>
              ...
              <div className="hidden md:flex bg-[#252630] p-5 rounded-xl border border-slate-700/50 shadow-sm flex-col justify-between"><div><p className="text-slate-400 text-sm font-medium mb-1">Forecast Tomorrow</p><div className="flex items-end gap-2"><h2 className="text-3xl font-bold text-white">{tomorrowForecast.yield.toFixed(1)}</h2><span className="text-slate-500 mb-1 font-medium text-xs">kWh</span></div></div><div className="mt-4 flex items-center gap-2 text-[10px] text-blue-500 font-bold uppercase tracking-widest"><Calendar className="w-3 h-3" /> 24h prediction</div></div>            </div>
            ...
            <div className="bg-[#252630] p-4 md:p-6 rounded-2xl border border-slate-700/50 shadow-lg">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6"><h2 className="text-lg font-semibold text-white flex items-center gap-2"><Activity className="w-5 h-5 text-indigo-400" /> Hourly Profile</h2><div className="flex flex-wrap gap-2">{[ { key: 'total', label: 'Total', color: '#fde047' }, { key: 'energy', label: 'Energy', color: '#818cf8' }, { key: 'strings', label: 'Strings', color: '#f59e0b' }, { key: 'uncertainty', label: 'Uncertainty', color: '#6366f1' } ].map(btn => (<button key={btn.key} onClick={() => toggleSeries(btn.key)} className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase border transition-all flex items-center gap-2 ${visibleSeries[btn.key] ? 'bg-slate-800 border-slate-600 text-white shadow-inner' : 'bg-transparent border-slate-800 text-slate-600'}`}><div className={`w-1.5 h-1.5 rounded-full transition-all ${visibleSeries[btn.key] ? 'scale-100' : 'scale-50 opacity-40'}`} style={{ backgroundColor: btn.color }}></div>{btn.label}</button>))}</div></div>
              <div className="h-[250px] w-full"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={selectedDayData} margin={{ top: 10, right: 40, left: 40, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#334155" /><XAxis dataKey="timeLabel" interval={3} stroke="#64748b" fontSize={11} axisLine={false} tickLine={false} /><YAxis stroke="#64748b" fontSize={11} axisLine={false} tickLine={false} label={{ value: 'POWER (kW)', angle: -90, position: 'insideLeft', offset: -30, dy: 0, fontSize: 8, fontWeight: 'bold', fill: '#64748b' }} /><YAxis yAxisId="right" orientation="right" stroke="#818cf8" fontSize={10} axisLine={false} tickLine={false} unit="kWh" label={{ value: 'ENERGY (kWh)', angle: 90, position: 'insideRight', offset: -30, dy: 0, fontSize: 8, fontWeight: 'bold', fill: '#64748b' }} /><Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px' }} content={({ active, payload, label }) => { if (active && payload && payload.length) return (<div className="bg-[#1e293b] border border-slate-700 p-3 rounded-lg shadow-xl text-[10px] space-y-1"><p className="font-bold text-slate-400 mb-1">{label}</p>{payload.map((e, idx) => (<div key={idx} className="flex justify-between gap-4"><span style={{ color: e.color }} className="font-bold">{e.name}:</span><span className="text-white font-mono">{e.value} {e.dataKey === 'cumulativeYield' ? 'kWh' : 'kW'}</span></div>))}<div className="mt-1 pt-1 border-t border-slate-800 text-[8px] text-slate-500 italic">Uncertainty based on {payload[0]?.payload?.cloudCover}% cloud cover</div></div>); return null; }} />{visibleSeries.uncertainty && <Area type="monotone" dataKey="pRange" stroke="none" fill="#fde047" fillOpacity={0.3} name="Uncertainty" />}{visibleSeries.total && <Area type="monotone" dataKey="total" name="Total Power" stroke="#fde047" fill="#fde047" fillOpacity={0.1} strokeWidth={2} />}{visibleSeries.strings && (config.strings || []).map((s, idx) => <Line key={s.id} type="monotone" dataKey={`stringPowers.${s.id}`} name={s.name} stroke={STRING_COLORS[idx % STRING_COLORS.length]} strokeWidth={1} dot={false} strokeDasharray="5 5" />)}{visibleSeries.energy && <Line yAxisId="right" type="monotone" dataKey="cumulativeYield" name="Energy" stroke="#818cf8" strokeWidth={3} dot={false} />}{currentHourTick && <ReferenceLine x={currentHourTick} stroke="#818cf8" strokeDasharray="4 4" />}</ComposedChart></ResponsiveContainer></div>
            </div>

            <div className="bg-[#252630] rounded-2xl border border-slate-700/50 shadow-lg overflow-hidden"><div className="grid grid-cols-1 md:grid-cols-2"><div className="p-6 border-b md:border-b-0 md:border-r border-slate-800"><h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4 flex items-center gap-2"><Zap className="w-3 h-3 text-amber-400" /> Real-time</h3><div className="space-y-4"><div className="flex justify-between items-center"><span className="text-xs text-slate-400">Forecast This Hour</span><span className="text-sm font-bold text-white">{(vitals.thisHour * 1000).toLocaleString()} <span className="text-[10px] text-slate-500 font-normal">Wh</span></span></div><div className="flex justify-between items-center"><span className="text-xs text-slate-400">Next Hour</span><span className="text-sm font-bold text-white">{(vitals.nextHour * 1000).toLocaleString()} <span className="text-[10px] text-slate-500 font-normal">Wh</span></span></div></div></div><div className="p-6 bg-slate-900/20"><h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4 flex items-center gap-2"><Calendar className="w-3 h-3 text-indigo-400" /> Daily</h3><div className="space-y-4"><div className="flex justify-between items-center"><span className="text-xs text-slate-400">Remaining Today</span><span className="text-sm font-bold text-indigo-400">{vitals.remainingToday.toFixed(2)} <span className="text-[10px] text-slate-500 font-normal">kWh</span></span></div><div className="flex justify-between items-center"><span className="text-xs text-slate-400">Tomorrow</span><span className="text-sm font-bold text-white">{tomorrowForecast.yield.toFixed(2)} <span className="text-[10px] text-slate-500 font-normal uppercase">kWh</span></span></div></div></div></div></div>
          </div>
        )}
        ...
        {activeTab === 'history' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 pb-8">
            {isDemo ? (
              <div className="bg-[#252630] p-12 rounded-3xl border border-slate-700 shadow-xl text-center space-y-6">
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="bg-[#252630] p-4 rounded-xl border border-slate-800"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Average Accuracy</p><div className="flex items-end gap-1"><h3 className="text-xl font-bold text-white">±{avgError}%</h3></div></div>
                  <div className="bg-[#252630] p-4 rounded-xl border border-slate-800"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Tracking Days</p><h3 className="text-xl font-bold text-indigo-400">{daysEntered}</h3></div>
                  <div className="bg-[#252630] p-4 rounded-xl border border-slate-800"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Best Day</p>{bestDay ? (<div className="text-[10px] font-bold text-emerald-400 leading-tight">{bestDay.date.toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' })} &bull; {(bestDay.absDelta * 100).toFixed(1)}% off</div>) : <span className="text-slate-600 text-[10px]">--</span>}</div>
                  <div className="bg-[#252630] p-4 rounded-xl border border-slate-800"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Worst Day</p>{worstDay ? (<div className="text-[10px] font-bold text-amber-400 leading-tight">{worstDay.date.toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' })} &bull; {(worstDay.absDelta * 100).toFixed(1)}% off</div>) : <span className="text-slate-600 text-[10px]">--</span>}</div>
                </div>
