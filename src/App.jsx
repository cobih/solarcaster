      ...
      <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6">
        ...
        {showConfig && (
          <div className="bg-[#252630] p-5 rounded-xl border border-slate-700 shadow-lg animate-in fade-in slide-in-from-top-4 space-y-6">
            ...
            <div className="pt-4 border-t border-slate-700/50 flex flex-col gap-4"><div className="flex justify-between items-center text-xs text-slate-400"><p>System Capacity: <strong className="text-white text-sm">{totalCapacity.toFixed(2)} kWp</strong></p><button onClick={handleLogout} className="text-red-400 hover:underline md:hidden text-xs font-bold uppercase tracking-widest">Log Out</button></div><div className="bg-[#1a1b23] p-4 rounded-2xl border border-slate-800 space-y-3"><div className="flex justify-between items-center"><div className="flex items-center gap-2 text-indigo-400 font-black text-[10px] uppercase tracking-widest"><Zap className="w-3 h-3" /> External API (Smart Home)</div><button onClick={() => saveConfigToCloud({ ...config, apiEnabled: !config.apiEnabled })} className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all ${config.apiEnabled ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-800 text-slate-500 border border-slate-700'}`}>{config.apiEnabled ? "Enabled" : "Disabled"}</button></div>{config.apiEnabled ? (<div className="space-y-2"><p className="text-[10px] text-slate-500 leading-tight">Home Assistant URL:</p><div className="flex gap-2"><input readOnly value={`https://firestore.googleapis.com/v1/projects/solar-forecaster-63320/databases/(default)/documents/public_forecasts/${user?.uid}`} className="flex-1 bg-black/30 border border-slate-800 rounded px-2 py-1.5 text-[9px] font-mono text-indigo-300 outline-none" /><button onClick={() => { navigator.clipboard.writeText(`https://firestore.googleapis.com/v1/projects/solar-forecaster-63320/databases/(default)/documents/public_forecasts/${user?.uid}`); alert("API URL copied!"); }} className="px-2 bg-indigo-600 rounded text-[9px] font-bold text-white uppercase">Copy</button></div></div>) : (<p className="text-[9px] text-slate-600 italic leading-tight">Enable to generate a public JSON endpoint for your smart home.</p>)}</div><div className="p-2 bg-slate-900/50 rounded border border-slate-800 text-[9px] font-mono text-slate-600 flex justify-between tracking-tighter uppercase font-bold px-3"><p>UID: {user?.uid?.slice(0,8) || "DEMO"}...</p><p>DB: {dbStatus} | Sync: {lastSynced || "Never"}</p></div></div>
          </div>
        )}
        ...
        {activeTab === 'history' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 pb-8">
            ...
            <div className="bg-[#252630] rounded-2xl border border-slate-700/50 overflow-hidden shadow-lg"><div className="p-4 border-b border-slate-700/50 bg-[#1e293b]/50 flex justify-between items-center"><h2 className="text-sm font-black text-white uppercase tracking-widest">Historical Production</h2><span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest italic">kWh Reading</span></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-[#1e293b]/50 border-b border-slate-700"><tr className="text-slate-400 uppercase text-[10px] font-black tracking-widest"><th className="p-4">Date</th><th className="p-4">Model</th><th className="p-4 text-indigo-400">Actual</th></tr></thead><tbody className="text-slate-300 divide-y divide-slate-700/50">{dailyTotals.map((day, i) => (<tr key={i} className="hover:bg-[#2d2e3a] transition-colors"><td className="p-4 font-bold text-xs whitespace-nowrap">{day.date.toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' })}</td><td className="p-4 font-bold text-white text-xs">{day.yield.toFixed(2)} <span className="text-[10px] text-slate-500 font-normal">kWh</span></td><td className="p-4"><div className="flex items-center gap-2"><input type="number" value={actuals[day.dayLabel] || ''} onChange={e => { saveActualToCloud(day.dayLabel, e.target.value); logAnalyticsEvent('actual_entry', { day: 'history' }); }} className="w-20 h-9 bg-[#1a1b23] border border-slate-600 rounded-lg px-2 text-white text-sm focus:border-indigo-500 outline-none transition-all shadow-inner font-mono" placeholder="0.0" /><Zap className="w-3 h-3 text-slate-600" /></div></td></tr>))}</tbody></table></div></div>
          </div>
        )}
