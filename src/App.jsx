        ...
        {showConfig && (
          <div className="bg-[#252630] p-5 rounded-xl border border-slate-700 shadow-lg animate-in fade-in slide-in-from-top-4 space-y-6">
            <div className="space-y-4 pb-6 border-b border-slate-700/50">
              ...
              {locationMode === 'search' && (<div className="relative animate-in fade-in slide-in-from-left-2"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" /><input type="text" value={addressQuery} onChange={e => searchAddress(e.target.value)} placeholder="Search address, postal code, or city..." className="w-full pl-10 pr-4 py-2.5 bg-[#1a1b23] border border-slate-600 rounded-lg text-sm text-white focus:border-indigo-500 outline-none transition-all" />{searchLoading && <Activity className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-500 animate-spin" />}{searchResults.length > 0 && (<div className="absolute z-50 mt-2 w-full bg-[#1a1b23] border border-slate-700 rounded-xl shadow-2xl overflow-hidden">{searchResults.map(r => (<button key={r.id} onClick={() => selectLocation(r)} className="w-full px-4 py-3 text-left text-sm text-slate-300 hover:bg-indigo-600/20 hover:text-white border-b border-slate-800 last:border-0 transition-colors flex items-center gap-3"><Navigation className="w-3 h-3 text-indigo-400" /><div><div className="font-bold">{r.name}</div><div className="text-[10px] text-slate-500">{r.country}</div></div></button>))}</div>)}</div>)}
            ...
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              ...
              <button onClick={addString} className="px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-all active:scale-95 shadow-sm">
                <Plus className="w-3 h-3" /> Add String
              </button>
            </div>
            
            <div className="bg-[#1a1b23] p-3 rounded-xl border border-slate-800 flex items-center gap-4">
               <label className="text-[9px] font-black text-slate-500 uppercase shrink-0">Efficiency</label>
               <input 
                 type="range" 
                 min="10" max="100" 
            ...
            <div className="bg-[#1a1b23] p-5 rounded-2xl border border-slate-800 space-y-6">
              ...
              <div className="space-y-4 pt-2">
                <div className="flex justify-between items-center bg-[#252630] p-3 rounded-xl border border-slate-700">
                  <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wider">Enable Export Earnings?</span>
